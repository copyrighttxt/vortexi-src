...
def ReadAccessKey():
    """ Computer\HKEY_LOCAL_MACHINE\SOFTWARE\WOW6432Node\ROBLOX Corporation\Roblox\AccessKey coon """
    try:
...
def thumbnailQueueWorker( workerNumber: int ):
    ...
    InstanceController : RccController = GetNextAvailableRCCInstance( version = "2021", startKillerWatcherThread = False)
    RCCRenders : int = 0
    while True:
        try:
            ...
            ThumbnailIs3D = ThumbnailRequestInfo.get("Is3D") or False 
                ...
                ExecuteJSON = {
                    ...
                    "Settings": {
                        ...
                        "Arguments": [
                            ...
                            config.BaseURL,
                            "PNG" if not ThumbnailIs3D else "OBJ",
            ...
            uploadReq = requests.post(
                f"http://127.0.0.1:{str(config.CommPort)}/ThumbnailReturn?RCCReturnAuth={RCCReturnAuth}",
                data = Base64Image + "|" + str(ThumbnailRequestInfo['reqid']) + "|" + str(ThumbnailRequestInfo['starttime']) + "|" + str(ThumbnailIs3D)
            )
            #InstanceController.KillRCC()
...
def ThumbnailReturn():
    ...
    try:
        ...
        Is3D = len(data) > 3 and data[3] == "True"
        
        if not Is3D:
            binary_data = base64.b64decode(imgdata)
        else:
            binary_data = imgdata.encode('utf-8') if isinstance(imgdata, str) else imgdata
        
        logging.info(f"Thumbnail returned for request {reqid}, took: {str(round(time.time() - startime, 2))} seconds, Is3D: {Is3D}")
        ...
        req = requests.post(
            ...
            headers={
                ...
                "ReturnUUID": reqid,
                "Content-Type": "application/json" if Is3D else "image/png"
            },
            data=binary_data
        )
        return "OK", 200
...
@app.route("/Avatar3D", methods=["POST"])
def Avatar3D():
    # Expected json data:
    # {
    #   "userid": 1,
    ...
    try:
        data = request.json
        data['starttime'] = time.time()
        data['type'] = 18  # a new type for 3d off topicly
        data['image_x'] = 768  
        data['image_y'] = 768
        thumbnailQueue.put(data)
        logging.info(f"3D Avatar request queued, id: {data['reqid']}, userid: {data['userid']}")
        return "OK", 200
    except Exception as e:
        logging.error(f"Error in Avatar3D: {e}")
        return "Error", 500
